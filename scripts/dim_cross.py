...
from main_tree import IsingModel, simulate_ising_model, plot_graph
...
### Run the Ising model on the stacked branch
model = IsingModel(nodes, neighbors, temp=T, J=J, G=stacked_graph, n_equilib_steps=n_equilib_steps, n_mcmc_steps=n_mcmc_steps, n_samples=n_samples, n_sample_interval=1, step_algorithm="wolff")
model = simulate_ising_model(model)
#animate_ising_model(model, output_dir=output_dir)

### plot the final graph
ax = plot_graph(model.nodes,neighbors, G=stacked_graph, draw_edges=False)
ax.set_title(f"T={T}")
...
for i, ax in enumerate(axs):
    ...
    plot_graph(model.nodes[i*nodes_per_layer:(i+1)*nodes_per_layer], neighbors[i*nodes_per_layer:(i+1)*nodes_per_layer], G=layer_graph, draw_edges=False, ax=ax)
...
for T in temps:
    ...
    model = simulate_ising_model(model)
    magnetization.append(model.avg_magn)
